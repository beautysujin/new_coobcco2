    ...
    try:
        ...
        if start_get == "yes":
            ...
            if result_my_server_read == 'start':
                ...
                file_path = "C:\\my_games\\coobcco2\\data_od\\mymodule\\version.txt"
                with open(file_path, "r", encoding='utf-8-sig') as file:
...
def server_get_odin():
    try:
        url = "https://raw.githubusercontent.com/rntkdgnl932/server/master/odin.txt"

        response = requests.get(url, headers={'Cache-Control': 'no-cache'})
        # response = requests.get(url, headers={'Cache-Control': 'no-cache'})
        data = response.text

        print("server_get_odin", data)
        return data
    except Exception as e:
        print(e)
        return 0
...
def server_get_version():
    try:
        url = "https://raw.githubusercontent.com/beautysujin/new_coobcco2/master/data_od/mymodule/version.txt"
